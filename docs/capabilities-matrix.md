...
| OIDC strategy | `@qavo/core` | � | Authorization Code + PKCE flow end-to-end: pluggable token client, session-storage state store, callback component/route, silent renewal scheduler. Provider-agnostic; integrators can replace the token client with an SDK if desired. |
...
|---|:---:|---|
| Unit testing (Vitest) + harness | 🟢 | `@qavo/testing` harness, mocks, fixtures; baseline specs cover error mapping, plugins, feature flags, logging, trace, PKCE, OIDC state store, validators, server-error reconciliation, theme service, HTTP interceptors. |
...
| Independent package publishing | 🟢 | Tag-triggered GitHub Actions workflow builds, tests and publishes every `@qavo/*` package to npm with provenance, then opens a matching GitHub Release. See [release process](releasing.md) and [ADR 0009](adr/0009-release-automation.md). |
| Token reference docs | 🟢 | Auto-generated from the typed catalog into [`docs/token-reference.md`](token-reference.md); regenerated by `npm run docs:tokens`. |
...
- The **OIDC token client** ships with a standards-compliant default; non-standard
  IdP quirks may require replacing it with a vendor SDK adapter — the seam is
  designed for that.
